function [errorValue, delta_V, delta_W] = trainNeuralNet(Input, Output, V, W, delta_V, delta_W)
    ...
    for i = 1 : n
        for j = 1 : a
            d(i,j) = (Output(i,j) - Output_of_OutputLayer(i,j)) * ...
                Output_of_OutputLayer(i,j) * ...
                (1 - Output_of_OutputLayer(i,j));
        end
    end
    ...
    % So we have to initialize with zero.
    % The size of delta V and delta W will be same as that of V and W matrix
    % respectively (nargin - no of arguments).
    ...
    for i = 1 : m
        for j = 1 :n
            d_star(i,j)= error(i,j) * Output_of_HiddenLayer(i,j) * ...
                (1-Output_of_HiddenLayer(i,j));
        end
    end
