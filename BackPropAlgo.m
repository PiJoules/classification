function BackPropAlgo(Input, Output)
    ...
    [errorValue, delta_V, delta_W] = trainNeuralNet(...
        Norm_Input, Norm_Output, V, W);

    % Checking if error value is greater than 0.1. If yes, we need to train 
    % the network again. User can decide the threshold value.
    while errorValue > 0.05
        ...
        [errorValue, delta_V, delta_W] = trainNeuralNet(...
            Norm_Input, Norm_Output, V, W, delta_V, delta_W);
    end

    % This code will be executed when the error value is less than 0.05
    if errorValue < 0.05
